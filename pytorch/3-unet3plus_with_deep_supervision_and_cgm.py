...
class unet3plus(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.deep_sup == True:
            y1 = self.y1(d1) * cls
            y2 = F.interpolate(self.y2(d2), scale_factor=2, mode="bilinear", align_corners=True) * cls
            y3 = F.interpolate(self.y3(d3), scale_factor=4, mode="bilinear", align_corners=True) * cls
            y4 = F.interpolate(self.y4(d4), scale_factor=8, mode="bilinear", align_corners=True) * cls
            y5 = F.interpolate(self.y5(e5), scale_factor=16, mode="bilinear", align_corners=True) * cls
